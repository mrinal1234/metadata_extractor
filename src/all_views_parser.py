...
if __name__ == "__main__":
    ...
    for filename in glob.glob("*.sql"):  # ('ACQ_O.sql'):('01_check.sql')
        ...
        for line in lines:
            ...
            if len(m) != 0:
                ...
                if (
                    block.lower()
                    .strip()
                    .startswith(("select", "(select", "( select", "(\nselect", "with"))
                ):
                    # block = Parser(block).generalize
                    try:
                        table_list = Parser(
                            block
                        ).tables  # Parser(' '.join(block.split())).tables
        ...
        d2 = pd.DataFrame(
            d1,
            columns=["Schema", "View_Name", "Operation", "Table_Schema_Name", "Tables"],
        )
        d2.insert(0, "File", filename)
        d2 = d2[~(d2.Tables.isin(["SET", "UPDATE", "(SELECT", "_DT,", "MIN", "MAX"]))]
